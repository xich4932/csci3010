...
enum party void get_max_party(int arr[]){
...
void Election::report_win(){
    ...
    std::map<int, party> find_max;
    int idx_arr = 0;
    for(enum party party_name = party::one; party_name <= party::none; party_name = (party)(party_name+1)){
        if(party_name == party::none) continue;
        find_max.insert(std::pair<int, enum party>(sum_votes[i], party_name));
        idx_arr++;
    }
    int max_vote = find_max.rbegin()->first;
    enum party winner_party = find_max.rbegin()->second;
    std::cout << "Congratulations, Bart, you've won!"
