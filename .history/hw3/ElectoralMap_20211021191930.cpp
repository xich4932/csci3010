...
void Election::print_each_Vote(){
    for(auto i = candidate_.begin(); i != candidate_.end(); i++){
        std::cout << i->second->print_vote() << std::endl;
    }
}
...
void Election::voting(){
    std::cout << "starting voting" << std::endl;
        ...
        for(enum party party_name = one; party_name < none; party_name = (party)(party_name+1)){
            ...
            if(active_party[ party_name]){
                ...
                if(party_name == party::none){ //the majority constituent is still none
                    ...
                    std::cout << "replace for party none "<< none_cantitutent << std::endl;
                    int sum = 0;
    ...
    for(auto i = candidate_.begin(); i != candidate_.end(); i++){
        for(int d = 1; d < num_district; d++){
            std::cout << i->second->get_name() << i->second->get_vote(*vote_district[d]) << std::endl;
        }
