...
std::vector<int> Election::party_none = {};
...
int Election::active_party[3] = {0,0,0};


Candidate::Candidate(){
    ;
}
void Candidate::plus_vote(const int dis, int count){
    auto iter = votes.find(dis);
    if(iter == votes.end()){
...
Candidate::Candidate(int given_id, party party_name, std::string candidate_name){
    ...
    for(auto i = temp_map.begin(); i != temp_map.end(); i++){
        votes.insert(std::pair<int, int>(i->second->get_id(), 0));
    }
...
void Candidate::print_vote(){
    int count = 0;
    for(auto i = votes.begin(); i != votes.end(); i++){
      //  int id = i->first.get_id();
        std::cout << i->first << " "<< i->second << std::endl;
        count ++;
    }
}
...
Election::Election(){
    ...
    std::cout <<"party one" << active_party[0] << std::endl;
    std::cout <<"party two" << active_party[1] << std::endl;
    std::cout <<"party three" << active_party[2] << std::endl;
}
...
Candidate* Election::who_campaigning(){
    ...
    while ((1))
    {
        ...
        if(std::stoi(choice) <= ids && candidate_.find(std::stoi(choice)) == candidate_.end()){
            std::cout << "index not availble" <<std::endl;
            continue;
        } 
        break; //jump out of loop if id is availble
    }
    std::cout << candidate_[std::stoi(choice)]->get_name() << std::endl;
...
void Election::voting(){
    std::map<int, District*> temp_map = ElectoralMap::getInstance().get_map();
    std::vector<std::vector<int>> store_all = {party_one_active, party_two_active, party_three_active, party_none};
  //  std::cout << party_one_active.size() << std::endl;
   // std::cout << party_two_active.size() << std::endl;
   // std::cout << party_three_active.size() << std::endl;
    for(auto each_district = temp_map.begin(); each_district != temp_map.end(); each_district++){
       // std::cout << "rount "<< std::endl;
        for(enum party temp_party = one; temp_party <= none; temp_party = (party)(temp_party + 1)){
            std::cout << "temppar : " << temp_party << std::endl;
            if(temp_party == party::none) continue;
            int vote = each_district->second->get_constituent(temp_party);
            std::cout << temp_party <<" has voters "<< vote << std::endl;
            if(temp_party != party::none && store_all[temp_party].size() > 0){ //three party constituent
                std::cout << "!!!" << std::endl;
                int rand_candidate = rand()% store_all[temp_party].size();
                std::cout <<"rand1:  "<< store_all[temp_party][rand_candidate] << std::endl;
                std::cout << "district_id "<< each_district->second->get_id() << std::endl;
                candidate_[store_all[temp_party][rand_candidate]]->plus_vote(each_district->second->get_id(), vote);
            }else{ //who is party none
                enum party max_local = each_district->second->get_max_party();
                std::cout << "fre "<< max_local << std::endl;
                if(max_local != party::none && store_all[max_local].size() > 0){
                    std::cout << "max_local " << max_local << std::endl;
                    int rand_candidate = rand()% store_all[max_local].size();
                    std::cout <<"rand2:  "<< store_all[max_local][rand_candidate] << std::endl;
                    std::cout << "district_id "<< each_district->second->get_id() << std::endl;
                    candidate_[store_all[max_local][rand_candidate]]->plus_vote(each_district->second->get_id(), vote);
                }else{
                    std::cout << "^^^^^" << std::endl;
                    int rand_candidate = rand()% candidate_.size();
                    std::cout <<"rand3:  "<< store_all[temp_party][rand_candidate] << std::endl;
                   std::cout << "district_id "<< each_district->second->get_id() << std::endl ;
                    candidate_[rand_candidate]->plus_vote(each_district->second->get_id(), vote);
                }
...
void District::convert_constituent(party increase_party, party decrease_party, int num){
    ...
    //std::cout << "increase:" << map_party[increase_party] <<std::endl;
    //std::cout << "decrease: "<<map_party[decrease_party] <<std::endl;
...
void Election::converting(District * campaign_district, Candidate * this_candidate){
    ...
    double possibility = ((sum_constituent+1)*2/sum_residents)*((sum_constituent+1)*2/campaign_district->get_square_mile())*100;
   
    double p_success = std::min(100.00,possibility );
    double p_extra_success = 0.1 * p_success;
    double this_rand = rand()%100;
...
void Election::report_win(){
    ...
    for(auto i = print_map.begin(); i != print_map.end(); i++){
        ...
        for(int c = 1; c <= ids; c++){
            std::cout << candidate_[c]->get_name() <<" "<< candidate_[c]->get_vote(i->second->get_id()) << std::endl;
        }
    }
    Candidate *find_max = candidate_.begin();
    int max = candidate_.begin()->second->get_total_vote();
    for(auto i = candidate_.begin(); i != candidate_.end(); i++){
        if(i->second->get_total_vote() > max){
            find_max = i->second;
            max = i->second->get_total_vote();
        }
            
    }
    std::cout << "Congratulations, "<< find_max->get_name() <<", you've won!"<<std::endl;
...
/* void RepresentativeELection::voting(){
    ...
    std::vector<std::vector<int>> store_id_each = {party_one_active, party_two_active, party_three_active, party_none};
    ...
    for(int d = 1; d < num_district; d++){
        ...
        for(enum party party_name = party::one; party_name <= party::none; party_name = (party)(party_name+1)){
            if(party_name == party::none){
                std::cout << "catch none" << std::endl;
                ...
                if(party_name == party::none){ //the majority constituent is still none
                    ...
                    candidate_[get_voted].plus_vote(vote_district[d]->get_id(), vote_per_district[d]);
                }
            }else if(active_party[party_to_int(party_name)]){
                std::cout <<"active candidate" << std::endl;
                int get_voted = rand()%store_id_each[party_name].size();
                std::cout << "get_vopted "<< get_voted << std::endl;
                candidate_[party_one_active[get_voted]].plus_vote(vote_district[d]->get_id(), vote_per_district[d]);
            ...
            std::cout << vote_district[d]->get_id() << std::endl;
        }
    }
} */