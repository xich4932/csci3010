...
int Election::is_direct = true;
int RepresentativeElection::is_direct = false;
...
void Election::report_win(){
   ...
   /*  for(auto i = candidate_.begin(); i != candidate_.end(); i++){
    ...
    for(auto i = candidate_.begin(); i != candidate_.end(); i++){
        find_max.insert(std::pair<int, enum party>(i->get_vote(), i));
    }
    std::cout << "Congratulations, "<< find_max.rbegin()->second.get_name() <<", you've won!"<<std::endl;

}

void party_to_int(enum party temp){
...
void RepresentativeElection::voting(){
    std::map<int, District> vote_map = ElectoralMap::getInstance().get_map();
    int total_vote[num_enum-1] = {0};
    //std::map<party, int> map_representative;
    map_representative.insert(std::pair<party,int>(party::one,0)); 
    map_representative.insert(std::pair<party,int>(party::two,0)); 
    map_representative.insert(std::pair<party,int>(party::three,0)); 
