...
void RepresentativeELection::voting(){
    ...
    for(auto each_district = temp_map.begin(); each_district != temp_map.end(); each_district++){
        ...
        int this_rand = rand() % store_all[max_party];
        candidate_[store_all[max_party][this_rand]].plus_vote(each_district, each_district->second->get_id());
    }
}

void RepresentativeElection::report_win(){
    std::map<int, District*> print_map = ElectoralMap::getInstance().get_map();
    ...
    for(auto i = candidate_.begin(); i != candidate_.end(); i++){
        ...
        for(auto p = print_.begin(); p != print_.end(); p++){
            ...
            std::cout << i->second->get_name()<< " " << p->second << std::endl;
